...
class ___AssignTokensToPositionsJob extends BaseQueuableJob
{
    ...
    public function compute()
    {
        ...
        foreach ($positions as $position) {
            ...
            if ($fastTradedSymbol) {
                ...
                $selectedCategories->push($fastTradedSymbol->symbol->category_canonical);
                continue;
            }
            ...
            if ($eligibleExchangeSymbol) {
                ...
                $selectedCategories->push($eligibleExchangeSymbol->symbol->category_canonical);
                continue;
            }
            ...
            if ($orderedExchangeSymbols->isNotEmpty()) {
                ...
                $selectedCategories->push($fallbackSymbol->symbol->category_canonical);
            } else {
    ...
    protected function prioritizeByCategory($orderedExchangeSymbols, $preferredDirection, $selectedCategories)
    {
        $categories = Symbol::query()->select('category_canonical')->distinct()->get()->pluck('category_canonical');
        $categoryAllocations = $categories->mapWithKeys(function ($category) use ($orderedExchangeSymbols, $preferredDirection, $selectedCategories) {
            if ($selectedCategories->filter(fn($cat) => $cat == $category)->count() >= 2) {
                return [$category => collect()]; // Limit to 2 per category.
            }
